...
OPTIONS:\n\
  ...
  -d, --dimensions \"width_mm,height_mm\" The width & height of your display in\n\
                             ...
                             \n\
  -R, --resolution \"width x height:freq\" The width & height of your display in\n\
                             pixels. Useful if your GPU doesn't provide\n\
                             valid physical dimensions for your display.\n\
                             \n\
...
SEE ALSO:\n\
  ...
  Modified by:  ESIEESPACE\n\
  Source:  https://github.com/spacerouter/flutter-pi\n\
  License: MIT\n\
\n\
...
static int init_display(void)
{
	...
	if (!flutterpi.display.override_gpu_resolution)
	{
		...
		{
			printf("Mode : %u x %u \n", mode_iter->hdisplay, mode_iter->vdisplay);
...
static bool parse_cmd_args(int argc, char **argv)
{
	...
	{
		longopt_index = 0;
		opt = getopt_long(argc, argv, "+i:o:r:d:R:h", long_options, &longopt_index);
		...
		case 'R':;
			...
			ok = sscanf(optarg, "%ux%u:%u", &width, &height, &refresh_rate);
			if ((ok == 0) || (ok == EOF))
			{
				fprintf(stderr, "ERROR: Invalid argument for --resolution passed.\n%s", usage);
				return false;
			}
			...
			flutterpi.display.override_gpu_resolution = true;
